...
/// sendData
///////////////////////////////////
int CxNetwork::sendData(int p_iSocket, const void* p_ptData, unsigned short p_usSize)
{
    int l_ireturn = 0;
    X_ASSERT(p_ptData != nullptr);
    X_ASSERT(p_usSize > 0);
    X_ASSERT(p_iSocket > 0);

    l_ireturn = send(p_iSocket, p_ptData, p_usSize, 0);

    if (l_ireturn != p_usSize)
    {
